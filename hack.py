...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://data:hackweek@198.13.42.242/data?charset=utf8' 	
...
class Publish(db.Model): # 建立Model对象
	...
	id = db.Column(db.Integer, primary_key=True,nullable=False)  #活动的ID
...
def activity(*args,**kwargs):
	...
	if (request.method == 'POST'):
		...
		check_token = User.query.filter_by(username=username).first()#通过token进行验证
		#判断ID
		if not check_token: 
			return jsonify({
				"success":'false',
		...
		db.session.commit()
		c = Publish.query.filter_by(id=id).first()
		return jsonify({
			"success": "true",
			"id": "{}".format(id),
			})
		...
		images = request.args.get("images")
		publish = Publish.query.filter_by(id=id).first_or_404()  
		if not id:
			abort(400)
		if not publish:
			return jsonify({
		...
		#token = simple_cache.get(token)
		token = request.args.get("token")
		check_token_put = User.query.filter_by(username=username).first()
		user = Publish.query.filter_by(id=id).first()
